...
static int ngx_tcp_lua_shdict_set(lua_State *L);
static int ngx_tcp_lua_shdict_get(lua_State *L);
static int ngx_tcp_lua_shdict_expire(ngx_tcp_lua_shdict_ctx_t *ctx,
    ngx_uint_t n);
static ngx_int_t ngx_tcp_lua_shdict_lookup(ngx_shm_zone_t *shm_zone,
    ngx_uint_t hash, u_char *kdata, size_t klen,
    ngx_tcp_lua_shdict_node_t **sdp);
static int ngx_tcp_lua_shdict_set_helper(lua_State *L, int flags);
static int ngx_tcp_lua_shdict_add(lua_State *L);
static int ngx_tcp_lua_shdict_replace(lua_State *L);
static int ngx_tcp_lua_shdict_incr(lua_State *L);
static int ngx_tcp_lua_shdict_delete(lua_State *L);
static int ngx_tcp_lua_shdict_flush_all(lua_State *L);
static int ngx_tcp_lua_shdict_flush_expired(lua_State *L);


#define NGX_TCP_LUA_SHDICT_ADD         0x0001
#define NGX_TCP_LUA_SHDICT_REPLACE     0x0002
...
void
ngx_tcp_lua_shdict_rbtree_insert_value(ngx_rbtree_node_t *temp,
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ngx_rbtree_node_t          **p;
    ngx_tcp_lua_shdict_node_t   *sdn, *sdnt;
    ...
    for ( ;; ) {
        ...
        if (node->key < temp->key) {
            ...
            sdn = (ngx_tcp_lua_shdict_node_t *) &node->color;
            sdnt = (ngx_tcp_lua_shdict_node_t *) &temp->color;
...
static ngx_int_t
ngx_tcp_lua_shdict_lookup(ngx_shm_zone_t *shm_zone, ngx_uint_t hash,
    u_char *kdata, size_t klen, ngx_tcp_lua_shdict_node_t **sdp)
{
    ngx_int_t                    rc;
    ngx_time_t                  *tp;
    uint64_t                     now;
...
void
ngx_tcp_lua_inject_shdict_api(ngx_tcp_lua_main_conf_t *lmcf, lua_State *L)
{
    ngx_tcp_lua_shdict_ctx_t   *ctx;
    ngx_uint_t                   i;
    ngx_shm_zone_t             **zone;
    ...
    if (lmcf->shm_zones != NULL) {
        ...
        lua_pushcfunction(L, ngx_tcp_lua_shdict_get);
        lua_setfield(L, -2, "get");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_set);
        lua_setfield(L, -2, "set");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_add);
        lua_setfield(L, -2, "add");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_replace);
        lua_setfield(L, -2, "replace");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_incr);
        lua_setfield(L, -2, "incr");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_delete);
        lua_setfield(L, -2, "delete");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_flush_all);
        lua_setfield(L, -2, "flush_all");

        lua_pushcfunction(L, ngx_tcp_lua_shdict_flush_expired);
...
static int
ngx_tcp_lua_shdict_get(lua_State *L)
{
    int                          n;
    ngx_str_t                    name;
    ngx_str_t                    key;
...
static int
ngx_tcp_lua_shdict_delete(lua_State *L)
{
    int             n;
    ...
    return ngx_tcp_lua_shdict_set_helper(L, 0);
}


static int
ngx_tcp_lua_shdict_flush_all(lua_State *L)
{
    ngx_queue_t                 *q;
    ngx_tcp_lua_shdict_node_t  *sd;
    int                          n;
...
static int
ngx_tcp_lua_shdict_add(lua_State *L)
{
    return ngx_tcp_lua_shdict_set_helper(L, NGX_TCP_LUA_SHDICT_ADD);
}


static int
ngx_tcp_lua_shdict_replace(lua_State *L)
{
    return ngx_tcp_lua_shdict_set_helper(L, NGX_TCP_LUA_SHDICT_REPLACE);
}


static int
ngx_tcp_lua_shdict_set(lua_State *L)
{
    return ngx_tcp_lua_shdict_set_helper(L, 0);
}
...
static int
ngx_tcp_lua_shdict_incr(lua_State *L)
{
    int                          n;
    ngx_str_t                    key;
    uint32_t                     hash;
...
ngx_int_t
ngx_tcp_lua_shared_dict_get(ngx_shm_zone_t *zone, u_char *key_data,
    size_t key_len, ngx_tcp_lua_value_t*value)
{
    u_char                      *data;
    size_t                       len;
    uint32_t                     hash;
    ...
    rc = ngx_tcp_lua_shdict_lookup(zone, hash, key_data, key_len, &sd);
...
ngx_shm_zone_t *
ngx_tcp_lua_find_zone(u_char *name_data, size_t name_len)
{
    ngx_str_t                       *name;
    ngx_uint_t                       i;
    ngx_shm_zone_t                  *zone;
