...
static int ngx_tcp_lua_var_set(lua_State *L);


void
ngx_tcp_lua_inject_variable_api(lua_State *L)
{
    /*  register reference maps */
    lua_newtable(L);    /* ngx.var */

    lua_createtable(L, 0, 2 /* nrec */); /* metatable for .var */
    lua_pushcfunction(L, ngx_tcp_lua_var_get);
    lua_setfield(L, -2, "__index");
    lua_pushcfunction(L, ngx_tcp_lua_var_set);
